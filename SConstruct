...
env.Append(CPPPATH=["src/", "libbf/"])

# Compiler libbf en C (pas C++)
libbf_sources = env.SharedObject("libbf/libbf.c")
libbf_cutils = env.SharedObject("libbf/cutils.c")

# Collects all .cpp files in the 'src' folder as compile targets.
sources = Glob("src/*.cpp") + [libbf_sources, libbf_cutils]
